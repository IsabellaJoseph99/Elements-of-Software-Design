...
#  Description: A group of recursive functions that determines if a list sums to a given target
...
def splitArrayHelper(start, nums, target):
    if (start >= len(nums)):
        ...
        return splitArrayHelper(start + 1, nums, target - nums[start]) or splitArrayHelper(start + 1,nums, target)
...
def splitOdd10Helper(start, nums, target):
    if (start >= len(nums)):
        return (target == 0)
    elif (nums[start] % 10 == 0):
        return splitOdd10Helper(start + 1, nums, target - nums[start])
    else:
        return splitOdd10Helper(start + 1, nums, target - nums[start]) or splitOdd10Helper(start + 1, nums, target)


def splitOdd10(nums):
    start = 0
    target1 = (sum(nums)//10)*10
    target2 = sum(nums) - target1
    if target2 % 2 == 1:
        return splitOdd10Helper(start, nums, target1)
    else:
        return False
...
def split53Helper(start, nums, mult5, mult3):
    ...
    elif (split53Helper(start+1, nums, mult5+nums[start], mult3)):
        return True
    elif (split53Helper(start+1, nums, mult5, mult3+nums[start])):
        return True
    else:
        return False
